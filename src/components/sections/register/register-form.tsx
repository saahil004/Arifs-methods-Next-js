...
export default function RegisterForm() {
  ...
  useEffect(() => {
    ...
    async function loadCourses() {
      try {
        ...
        // Subjects are optional context for the team, not required to
        // register — fail quietly and let the rest of the form still work.
      } finally {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("success");
      trackEvent("generate_lead", { level: levelLabel });
      trackMetaEvent("Lead");
      form.reset();
      setLevel("");
    } catch {
          ...
          >
            ...
            <div className="rounded-xl border border-navy/15 p-4">
              {coursesLoading ? (
                ...
                <p className="text-sm text-navy/40">No subjects found for this level.</p>
              ) : (
                visibleCourses.map((course) => (
                  <label key={course.id} className="flex items-center gap-2 py-1 text-sm text-navy/70">
                    <input
                      type="checkbox"
                      name="subjects"
